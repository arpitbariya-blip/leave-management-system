...
<head>
    ...
    if (isset($_POST["btnSubmit"])) {
        if ('Submit' == $_POST["btnSubmit"]) {
            ...
            $LeaveCount = isset($_POST["LeaveCount"]) ? $_POST["LeaveCount"] : 0;
            $Year = isset($_POST["Year"]) ? $_POST["Year"] : 0;
            $stmt = $conn->prepare("INSERT INTO facultyleavemaster (FacultyInfoID, TypeOfLeaveID, LeaveCount, YearID) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("isii", $FacultyName, $LeaveType, $LeaveCount, $Year);
                    ...
                    <div class="form-group">
                        ...
                        <select id="DeptID" name="DeptName" class="form-control">
                            ...
                            while ($data = mysqli_fetch_array($result)) {
                                ...
                                if (isset($_POST['DeptID']) && $data[0] == $_POST['DeptID']) {
                ...
                <div class="col-sm-6" id="AB" style="display: none;">
                    <div class="form-group">
                        ...
                        <select id="Year" name="Year" class="form-control" onchange="yearwise()">
                            ...
                            <?php
                            while ($data = mysqli_fetch_array($result)) {
                                $safeId = htmlspecialchars($data[1], ENT_QUOTES, 'UTF-8');
                                if (isset($_POST['YEAR']) && $data[1] == $_POST['YEAR']) {
                                    echo "<option value='$safeId' selected> $safeId </option>";
                                } else {
                                    echo "<option value='$safeId'> $safeId </option>";
                                }
                            }
                                                ...
                                                <tr>
                                                    ...
                                                    foreach ($data as $row) {
                                                        if ($row['LeaveType'] != "HCL") {
                                                            echo "<th>" . htmlspecialchars($row['LeaveType'], ENT_QUOTES, 'UTF-8') . "</th>";
