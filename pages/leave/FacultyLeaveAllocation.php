...
<head>
    ...
    if (isset($_POST["btnSubmit"])) {
        if ('Submit' == $_POST["btnSubmit"]) {
            ...
            $stmt = $conn->prepare("INSERT INTO facultyleaveallocation (FacultyInfoID, TypeOfLeaveID, Date, DeptID) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("issi", $FacultyName, $LeaveType, $Date, $DeptName);
                        ...
                        <div class="form-group">
                            ...
                            <select name="LeaveType" required="" class="form-control" value="<?php echo $TypeOfLeaveID; ?>"
                                ...
                                while ($data = mysqli_fetch_array($result)) {
                                    ...
                                    if (isset($_POST['LeaveTypeID']) && $data[0] == $_POST['LeaveTypeID']) {
                ...
                <div class="form-group">
                    ...
                    <select name="FacultyName" required="" id="SS" class="form-control"
                        ...
                        while ($data = mysqli_fetch_array($result)) {
                            $safeId = htmlspecialchars($data[0], ENT_QUOTES, 'UTF-8');
                            $safeName = htmlspecialchars($data[1], ENT_QUOTES, 'UTF-8');
                            if (isset($_POST['FacultyID']) && $data[0] == $_POST['FacultyID']) {
                                echo "<option value='$safeId' selected> $safeName </option>";
                            } else {
                                echo "<option value='$safeId'> $safeName </option>";
                            }
