...
<head>
    ...
    if (isset($_POST["btnSubmit"])) {
        if ('Submit' == $_POST["btnSubmit"]) {
            ...
            $stmt = $conn->prepare("INSERT INTO facultyleavemaster (FacultyInfoID, TypeOfLeaveID, LeaveCount, YearID, DeptID) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("isiii", $FacultyName, $LeaveType, $LeaveCount, $Year, $DeptName);
    ...
    if (isset($_POST["btnSubmit"])) {
        if ('Update' == $_POST["btnSubmit"]) {
            ...
            $stmt = $conn->prepare("UPDATE facultyleavemaster SET FacultyInfoID=?, TypeOfLeaveID=?, LeaveCount=?, YearID=?, DeptID=? WHERE FacultyLeaveMasterID=?");
            $stmt->bind_param("isiiii", $FacultyName, $LeaveType, $LeaveCount, $Year, $DeptName, $FacultyLeaveMasterID);
    ...
    if (empty($_GET["FacultyLeaveMasterID"])) {
        ...
        $btn = "Update";
        $stmt = $conn->prepare("SELECT * FROM facultyleavemaster WHERE FacultyLeaveMasterID=?");
                    ...
                    <div class="form-group">
                        ...
                        <select id="S1" name="DeptName" required="" class="form-control"
                            ...
                            while ($data = mysqli_fetch_array($result)) {
                                ...
                                if (isset($_GET['DeptID']) && $data[0] == $_GET['DeptID'] || $data[0] == $DeptID) {
