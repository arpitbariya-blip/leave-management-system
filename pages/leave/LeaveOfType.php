...
<head>
  ...
  if (isset($_POST["btnSubmit"])) {
    if ('Submit' == $_POST["btnSubmit"]) {
      ...
      $stmt = $conn->prepare("INSERT INTO typeofleave (LeaveType, Flag) VALUES (?, ?)");
      $stmt->bind_param("si", $LeaveType, $Flag);
  ...
  if (isset($_POST["btnSubmit"])) {
    if ('Update' == $_POST["btnSubmit"]) {
      $LeaveType = $_POST['LeaveType'];
      $stmt = $conn->prepare("UPDATE typeofleave SET LeaveType=? WHERE TypeOfLeaveID=?");
      $stmt->bind_param("si", $LeaveType, $_GET['TypeOfLeaveID']);
      if ($stmt->execute()) {
  ...
  if (empty($_GET['TypeOfLeaveID'])) {

  } else {
    $btn = "Update";
    $stmt = $conn->prepare("SELECT * FROM typeofleave WHERE TypeOfLeaveID=?");
      ...
      <div class="row">
        <div class="col-sm-12">
          <div class="form-group">
            ...
            <input type="text" class="form-control" value="<?php echo htmlspecialchars($LeaveType, ENT_QUOTES, 'UTF-8'); ?>" name="LeaveType" maxlength="50"
              id="LeaveType" placeholder="Leave Type  ">
          </div>
        </div>
